...
description: 'Learn how to use advanced computation features including math calculations, date operations, end-of-week, and end-of-month utilities in your custom fields'
tag: 'beta'
---
...
<Info>
  **What you'll learn:** How to perform mathematical calculations, date manipulations, end-of-week calculations, and end-of-month calculations directly within your custom fields using `mathutil`, `dateutil`, `end_of_week`, and `end_of_month` utilities.
</Info>
...
You can combine math, date, end-of-week, and end-of-month utilities in the same prompt for sophisticated calculations:
...
### **For Date Utilities (dateutil, end_of_week, end_of_month):**

<Warning>
  **Mandatory Fields for Date Operations:** When using any date utilities (`dateutil`, `end_of_week`, or `end_of_month`), both `{today}` and `{time_zone}` fields are **mandatory** and must be included in your prompt. During testing or when uploading audience data, you **must** provide the `time_zone` field, while `today` will be filled automatically by Vodex.
</Warning>
...
5. **Combine all utilities** - Create sophisticated prompts with multiple computation types
6. **Monitor and optimize** - Track performance and accuracy
